...
const convertDurationMinSecToMs = function (durationString) {
    let [min, sec] = durationString.split(':')
...
const convertKeyToKeyModeInt = function (keyString) {
    ...
    for (const keyValue in keyIntMap) {
        if (keyIntMap[keyValue] === keyString) {
            return [parseInt(keyValue), mode]
        }
    }
}


const convertKeyToInt = function (key, direction) {
...
module.exports = {
    convertDurationMinSecToMs,
    convertDurationToMinSec,
    convertKeyToKeyModeInt,
    convertIntToKey: convertKeyToInt,
    getAudioFeatures
}