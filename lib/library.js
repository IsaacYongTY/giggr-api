...
const convertEnharmonic = (key) => {
    let note = key.replace(/[#b]/g,"")
    const accidental = key[key.length - 1]

    if(!/[A-Ga-g]/g.test(note) || note.length !== 1) {
        return
    }
    ...
    if(accidental === '#') {
        return Object.keys(enharmonicPair).find(element => enharmonicPair[element] === key)
    }

    return enharmonicPair[key]
